...
namespace sha3 {
    ...
    void sha3_256(const uint8_t *in, size_t inlen, uint8_t *md) {
        ...
        for(; inlen >= rsizb; inlen -= rsizb, in += rsizb) {
            for(i = 0; i < rsizb / 8; i++) st[i] ^= ((uint64_t *)in)[i];
            keccakf(st);
        }
        memcpy(temp, in, inlen);
        temp[inlen++] = 0x06;
        memset(temp + inlen, 0, rsizb - inlen);
        temp[rsizb - 1] |= 0x80;
        for(i = 0; i < rsizb / 8; i++) st[i] ^= ((uint64_t *)temp)[i];
        keccakf(st);
        for(i = 0; i < 4; i++) ((uint64_t *)md)[i] = st[i];
    }
}
